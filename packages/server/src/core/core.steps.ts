...
import {
  ...
  stepWithWorld,
} from 'src/test_common/commonServerSteps';
...
When(
  ...
  stepWhichUpdatesWorld((world) => {
    ...
    return {
      ...world,
      request: server.get('/api/test').set('x-kafka-management-ui-request', ID),
      context: {
        ...context,
        requestId: ID,
      },
...
Then(
  ...
  stepWithWorld((world) => {
    const { request } = world;
    return request.expect('x-kafka-management-ui-request', /^.{16}$/).expect(418);
  })
);
...
Then(
  ...
  stepWithWorld((world) => {
    ...
    return request
      .expect('x-kafka-management-ui-request', context.requestId as string)
      .expect(418);
  })
);
...
Then(
  ...
  stepWithWorld((world) => {
    ...
    return request.expect('x-kafka-management-ui-module', 'mockapi');
  })
);
