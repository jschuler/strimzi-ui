...
Feature: core module
    ...
    Scenario: When making a call with no kafka-management-ui header, one is added for later requests
        Given a 'mockapi_only' server configuration
        And I run an instance of the kafka-management-ui server
        When I make a request with no unique request header
        Then a unique request header is returned in the response

    Scenario: When making a call with a kafka-management-ui header, that header is used in the request
        Given a 'mockapi_only' server configuration
        And I run an instance of the kafka-management-ui server
        When I make a request with a unique request header
        Then the unique request header sent is returned in the response

    Scenario: When making a call to the kafka-management-ui server, the expected secuirty headers are present
        Given a 'mockapi_only' server configuration
        And I run an instance of the kafka-management-ui server
    ...
    Scenario: When making a call to the kafka-management-ui server, the expected session cookie is present
        Given a 'mockapi_only' server configuration
